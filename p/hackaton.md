# WarungOS — Hackathon Project Brief
> Grand theme: Digital solution for UMKM (Usaha Mikro, Kecil, dan Menengah) in Indonesia
> Build target: 2 weeks | Format: Progressive Web App (PWA) | Stack: React + Supabase + Claude AI
...
| AI Demand Forecast | Food | Claude API predicts what menu items to prepare based on past sales + day of week |
...
│   │   ├── claude.js   # Claude API wrapper functions
...
├── tailwind.config.js
└── .env                # SUPABASE_URL, SUPABASE_ANON_KEY, CLAUDE_API_KEY, FONNTE_TOKEN
```
...
## 14. Demo Script (Presentation Flow)

1. **Hook (30 sec):** "62 juta UMKM di Indonesia masih catat stok pakai buku tulis."
2. **Problem walkthrough (1 min):** Show the 4 pain points with real numbers (food waste %, cancellation %, missed orders)
3. **Live demo — Kasir:** Sell 3 products → stock auto-decreases → AI low-stock alert fires
4. **Live demo — Pembukuan:** Show today's profit summary generated by Claude in Bahasa Indonesia
5. **Live demo — Booking:** Add appointment → show WhatsApp reminder preview
6. **Live demo — AI Forecast:** Show tomorrow's demand prediction for a warung makan
7. **Metrics close:** Before vs After comparison table on screen
8. **PWA moment:** Open on phone → install to homescreen live on stage

---

## 15. Hackathon Constraints & Notes

- Build time: 14 days
- Team assumption: 2-3 developers
- Priority order: Kasir + Stock (Week 1) → AI features + Booking (Week 2)
- AI features are additive — app works without Claude API if key is missing (graceful degradation)
- All text in Bahasa Indonesia for authenticity
- No real Shopee/Tokopedia API integration needed — order inbox is manual entry only (reduces scope significantly)
- WhatsApp reminder can be mocked in demo if Fonnte setup takes too long
- Supabase free tier: 500MB database, 2GB bandwidth — more than enough for hackathon demo
